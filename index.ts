...
import { WatsonExporter, FileWriter, Kind, ProjectReference } from "@botmock/export";

/**
 * Generates `.json` file in `./output`.
 *
 * @example
 * ```shell
 * npm start
...
async function main(): Promise<void> {
  ...
  const exporter = new WatsonExporter({ token: process.env.BOTMOCK_TOKEN as string });
